...
(function(CKEDITOR) {
			...
			editor.widgets.add('section', {
				...
				upcast: function(el, data) {
					...
					if (el.children.length < 2 || !CKEDITOR.api.parser.isMediaFigure(el.children[1])) {
						el.add(new CKEDITOR.htmlParser.element('div', {
							'class': 'media'
						}), 1);
					} else {
						el.children[1].wrapWith(new CKEDITOR.htmlParser.element('div', {
							'class': 'media'
						}));
					}
	...
	CKEDITOR.on('dialogDefinition', function(ev) {
		...
		css.items = [
			[ev.editor.lang.common.notSet, '']
		].concat(Object.getOwnPropertyNames(ev.editor.config.section).map(function(item) {
